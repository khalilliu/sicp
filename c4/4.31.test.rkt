#lang racket/base
(require rackunit
         "lib/lazy-memo-eval.rkt")

; Test suites
(define eval-tests
  (test-suite
   "Tests for the meta-circular evaluator - basic evaluation" 
...
(define boolean-tests
  (test-suite 
   "Tests for the meta-circular evaluator - and and or" 
...
(define special-cond-tests
  (test-suite 
   "Tests for the meta-circular evaluator - cond with => clauses"
...
(define let-tests
  (test-suite 
   "Tests for the meta-circular evaluator - let"
   (test-case 
    "test let"
    (test-equal?
     "simple let"
...
(define named-let-tests
  ...
  (test-suite
   "Tests for the meta-circular evaluator - named-let"
...
(define internal-definition-tests
  (test-suite
   "Tests for the meta-circular evaluator - internal definitions via transformation"
...
(define letrec-tests
  (test-suite
   "Tests for the meta-circular evaluator - letrec"
...
(define lazy-memo-tests
  (test-suite 
   "Tests for the meta-circular evaluator - lazy and memo arguments"
   (test-case "lazy/memo argument tests"
              (interpret '(define (fib (n lazy-memo))
                            (cond ((= n 0) 0)
                                  ((= n 1) 1)
                                  (else (+ (fib (- n 1))
                                           (fib (- n 2)))))))
              (interpret '(define (f a (b lazy) c (d lazy-memo))
                            (- (+ a b) (* c d))))
              (test-equal? "simple lazy definition"
                           (interpret '(f 5 4 3 2))
                           3)
